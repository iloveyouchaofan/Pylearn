...
1-2、括号和其中的object
	...
	*类继承的概念：
	...
	5、多态
		依赖 类 作为参数的 函数 或者 方法，都可以不加修改地正常运行，
		同一个函数，多种状态，
		class_1()
		class_2()

		def printf(classname):
			classname.dosomethin() #类内有共通的属性和方法！
...
8、*类的特性
	多态的方法是利用了 类的共通性。
	继承父类的方法，并在函数中（直接同名调用）即可。（实际上，是被包含在内的！）
	记得len() 是调用对象的 __len__方法吗？这个就是从上面衍生出来的。
	而且有 __len__ 方法的对象 就可以称之为 like ，例如 __str__方法
	以上就是类的特性：继承和多态，函数！都是相关联的！
9、*定制类
	按照 8 中可以推出
	__ name __ 前后两个下划线，表示 这个变量为特殊变量，特殊的方法
		疑问：有多少特殊变量？他们都是什么？
	通过这些特殊变量的组合，就成为定制类
	而且可以使用于 特殊变量 对应的方法调用这些属性。
